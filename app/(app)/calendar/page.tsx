...
export default function CalendarPage() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>(initDate.toISOString().split("T")[0])
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-1 bg-white border border-[#E5E5E5] rounded-xl p-1">
          {["monthly","weekly"].map((v, i) => (
            <button
              key={v}
              onClick={() => setView(v as "monthly" | "weekly")}
              className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-all ${
                view === v ? "bg-[#EAB308] text-[#1A1A1A]" : "text-[#737373]"
              }`}
            >
              {i === 0 ? "월간" : "주간"}
            </button>
