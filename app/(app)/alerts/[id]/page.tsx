...
import { ArrowLeft, Sparkle, CalendarBlank, Clock, MapPin, Users, Tag } from "@phosphor-icons/react"
import { Avatar, StatusBar } from "@/components/tcm/shared"
import { MOCK_DETECTIONS } from "@/lib/mock-data"
import { getDetection, saveDetection, dismissDetection } from "@/lib/api"
...
export default function AlertDetailPage() {
  ...
  useEffect(() => {
    ...
    getDetection(id).then(data => {
      if (!data || data.error) { setError("감지 이벤트를 찾을 수 없습니다."); setLoading(false); return }
      setDet(data)
      setLoading(false)
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const saved = await saveDetection(det.id)
  ...
  if (error || !det) return (
    <div className="flex flex-col h-full bg-[#FAFAFA]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
        ...
        <button onClick={() => router.back()} className="px-5 py-2.5 rounded-xl bg-[#EAB308] text-[#1A1A1A] text-[13px] font-bold">돌아가기</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAFAFA] relative">
      ...
      <div className="flex-1 bg-black/30 relative flex flex-col justify-end">
        <button onClick={() => router.back()} className="absolute top-4 left-4 w-9 h-9 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
        ...
        <div className="bg-white rounded-t-3xl overflow-hidden max-h-[85%]">
          ...
          <div className="overflow-y-auto px-5 pb-6 flex flex-col gap-5">
            ...
            {det.extracted_participants && (det.extracted_participants as string[]).length > 0 && (
            ...
            <div className="flex gap-3">
              <button onClick={handleSaveAndEdit} disabled={saving || dismissing}
              ...
              <button onClick={handleSave} disabled={saving || dismissing}
            ...
            <button onClick={handleDismiss} disabled={saving || dismissing}
