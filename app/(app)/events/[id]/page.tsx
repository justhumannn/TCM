...
import {
  ...
  NotePencil, BellRinging, CaretDown, X, Spinner, Trash,
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("calendar_events").select("*").eq("id", id).single()
      .then(({ data, error }) => {
        if (error || !data) { setError("일정을 찾을 수 없습니다"); setLoading(false); return }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isNew) {
        ...
        // 알림 저장
        if (alertEnabled && data) {
          await supabase.from("event_alerts").insert({ event_id: data.id, minutes_before: alertMins })
        }
  ...
  async function handleDelete() {
    ...
    if (error) { setError("삭제 중 오류가 발생했습니다"); setDeleting(false); return }
    ...
    <div className="flex flex-col h-full bg-[#FAFAFA]">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#E5E5E5] overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#F5F5F5]">
            <NotePencil size={16} weight="fill" className="text-[#EAB308] flex-shrink-0" />
            <span className="text-[12px] text-[#737373] w-10 flex-shrink-0">제목</span>
            {editMode ? (
              <input
                value={title}
          ...
          {/* 날짜 */}
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#F5F5F5]">
            <CalendarBlank size={16} weight="fill" className="text-[#EAB308] flex-shrink-0" />
            <span className="text-[12px] text-[#737373] w-10 flex-shrink-0">날짜</span>
          ...
          {/* 시간 */}
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#F5F5F5]">
            <Clock size={16} weight="fill" className="text-[#EAB308] flex-shrink-0" />
            <span className="text-[12px] text-[#737373] w-10 flex-shrink-0">시간</span>
            <span className="flex-1 text-[13px] text-[#171717] font-medium">{fmtTime(event?.event_time ?? null) || "시간 없음"}</span>
            {editMode && <CaretDown size={13} className="text-[#A3A3A3]" />}
          </div>
          {/* 장소 */}
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#F5F5F5]">
            <MapPin size={16} weight="fill" className="text-[#EAB308] flex-shrink-0" />
            <span className="text-[12px] text-[#737373] w-10 flex-shrink-0">장소</span>
            {editMode ? (
              <input
                value={location}
          ...
          <div className="flex items-start gap-3 px-4 py-3.5">
            <NotePencil size={16} weight="fill" className="text-[#EAB308] flex-shrink-0 mt-0.5" />
            <span className="text-[12px] text-[#737373] w-10 flex-shrink-0 mt-0.5">메모</span>
            {editMode ? (
              <textarea
                value={memo}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {ALERT_OPTIONS.map((mins) => (
                    <button
                      key={mins}
                      onClick={() => setAlertMins(mins)}
                      className={`flex-1 py-1.5 rounded-xl text-[11px] font-semibold border transition-all ${
                        alertMins === mins ? "bg-[#EAB308] text-[#1A1A1A] border-[#EAB308]" : "bg-white text-[#737373] border-[#E5E5E5]"
                      }`}
                    >
                ...
                <div className="flex gap-2">
                  {IMPORTANCE_LEVELS.map(({ label, value, cls }) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded-xl text-[11px] font-semibold ${cls} ${
                        importance === value ? "ring-2 ring-[#EAB308]" : ""
                      }`}
                    >
                      {label}
                    </button>
      ...
      <div className="px-4 pb-6 pt-3 bg-white border-t border-[#E5E5E5] flex flex-col gap-2">
        {(editMode || isNew) && (
          ...
          >
            {saving && <Spinner size={18} className="animate-spin" />}
            저장하기
          </button>
        )}
        {!isNew && (
          <button
            ...
            disabled={deleting}
            className="text-[13px] text-red-400 text-center py-1 font-medium disabled:opacity-50"
          >
            {deleting ? "삭제 중..." : "일정 삭제"}
          </button>
        )}
